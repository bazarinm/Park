...
Animal::Animal(unsigned nutr, Coords pos, const Park& territory, int _FOV):
	FOV(_FOV),
...
bool Animal::seekFood() {
	for (size_t i = 0; i < sight.size(); ++i)
		for (size_t j = 0; j < sight[i].size(); ++j)
			sight[i][j] = 0;
...
bool Animal::seekPartner() {
	...
	while (!steps.empty() && !found) {
		...
		for (Coords next_step : next) {
			Coords relat_step = toRelative(next_step); //relative to sight
			if (!inSight(relat_step))
				continue;

			if (sight[relat_step.x][relat_step.y] == 0 && next_step != pos) {
				if (isPartner(territory[next_step])) {
					closest_food = next_step;
					found = true;
				}
				else if (isVacant(territory[next_step])) {
					sight[relat_step.x][relat_step.y] = step_n;
				}
...
bool Animal::seekEnemy() {
	...
	while (!steps.empty() && !found) {
		...
		for (Coords next_step : next) {
			...
			if (sight[relat_step.x][relat_step.y] == 0 && next_step != pos) {
				if (isEnemy(territory[next_step])) {
					closest_food = next_step;
					found = true;
				}
				else if (isVacant(territory[next_step])) {
